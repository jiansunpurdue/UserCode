...
#ifndef SNGLHFCAND_V2_H
#define SNGLHFCAND_V2_H

#include "snglhfcand.hh"

class snglhfcand_v2 : public snglhfcand
{
    private:
        //.. topology cut
        float fprob;
    ...
    public:
        ...
        float get_ffxy() const{return ffxy;}
        float get_ffxye() const{return ffxye;}
        float get_fflsxy() const{return fflsxy;}
        float get_falpha0xy() const{return falpha0xy;}

		float get_svx() const{return svx;}
		float get_svy() const{return svy;}
		float get_svz() const{return svz;}
		...
		void set_ffxy( const float in ) { ffxy = in;}
        void set_ffxye( const float in ) { ffxye = in;}
        void set_fflsxy( const float in ) { fflsxy = in;}
        void set_falpha0xy( const float in ) { falpha0xy = in;}

		void set_svx( const float in ) { svx = in;}
		void set_svy( const float in ) { svy = in;}
		void set_svz( const float in ) { svz = in;}

        ClassDef(snglhfcand_v2, 1)
