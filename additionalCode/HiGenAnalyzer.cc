...
struct HydjetEvent{
  ...
  Float_t vx_production[MAXPARTICLES];
  Float_t vy_production[MAXPARTICLES];
  Float_t vz_production[MAXPARTICLES];
...
//}
//
//bool decidesaveitornot( const reco::GenParticle p )
//{
//	bool tosaveit = false;
//	
//	//save if if it is a D meson
//	if( TMath::Abs( p.pdgId() ) > 400 && TMath::Abs( p.pdgId() ) < 500 ) return true;
//	
//	if( p.numberOfMothers() != 1 )  return false;
//
...
//	    bool Dmesonindaughters = decideDmesonindaughters( pcand );
...
vector<int> HiGenAnalyzer::getMotherIdx(edm::Handle<reco::GenParticleCollection> parts, const reco::GenParticle pin){
    ...
    for(UInt_t i = 0; i < parts->size(); ++i){
	    ...
	    bool tosaveit = false;
		const Candidate * pcand = &p;
	    tosaveit = decidesaveitornot( pcand );
        ...
        for (unsigned int idx=0; idx<p.numberOfDaughters(); idx++){
            ...
            if(fabs(p.daughter(idx)->pt()-pin.pt())<0.001 && fabs(p.daughter(idx)->eta()-pin.eta())<0.001 && fabs(p.daughter(idx)->phi()-pin.phi())<0.001) motherArr.push_back(savedidx);
        }
    }
...
vector<int> HiGenAnalyzer::getDaughterIdx(edm::Handle<reco::GenParticleCollection> parts, const reco::GenParticle pin){
    ...
    for(UInt_t i = 0; i < parts->size(); ++i){
        ...
        for(unsigned int idx=0; idx<p.numberOfMothers(); idx++){
            ...
            if(fabs(p.mother(idx)->pt()-pin.pt())<0.001 && fabs(p.mother(idx)->eta()-pin.eta())<0.001 && fabs(p.mother(idx)->phi()-pin.phi())<0.001) daughterArr.push_back(savedidx); 
        }
    }
...
HiGenAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(useHepMCProduct_){
    ...
    for(UInt_t i = 0; i < parts->size(); ++i){
	  ...
	  hev_.vx_production[hev_.mult] = p.vx();
	  hev_.vy_production[hev_.mult] = p.vy();
	  hev_.vz_production[hev_.mult] = p.vz();
...
//  cout << "gen pv x: " << vx << "  vy: " << vy << "   vz: " << vz << endl;
...
HiGenAnalyzer::beginJob()
{
  ...
  if(doAnalysis_){
    ...
    if(doParticles_){
	  ...
	  hydjetTree_->Branch("vx_production",hev_.vx_production,"vx_production[mult]/F");
	  hydjetTree_->Branch("vy_production",hev_.vy_production,"vy_production[mult]/F");
	  hydjetTree_->Branch("vz_production",hev_.vz_production,"vz_production[mult]/F");
