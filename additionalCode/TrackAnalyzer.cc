...
TrackAnalyzer::fillVertices(const edm::Event& iEvent){
  ...
  if(doSimVertex_){
    ...
    for (unsigned int i = 0 ; i< vertices->size(); ++i){
	  ...
	  if(  daughter >(*vertices)[greatestgenvtx].nDaughterTracks() &&fabs((*vertices)[i].position().z())<30 )
		  greatestgenvtx = i;
    }
	pev_.genPVtx=greatestgenvtx;
//	cout << " Gen Pv vertex: " << greatestgenvtx << "  x: " << (*vertices)[greatestgenvtx].position().x() << "  y: " << (*vertices)[greatestgenvtx].position().y() << "  z: " << (*vertices)[greatestgenvtx].position().z() << endl;
  }
...
TrackAnalyzer::beginJob()
{
  ...
  trackTree_->Branch("genPVtx",&pev_.genPVtx,"genPVtx/I");
